...
$(document).ready ->
	if $('#quick-checkout-button')[0]
		$('html').on 'click', '#quick-checkout-button', ->
			$('#properties-quick-checkout').show()
			$('#properties-quick-checkout #unit_id').focus()
			return

	$('html').on 'click', '#hide-overlay svg', ->
		$('.overlay').hide()
		return
